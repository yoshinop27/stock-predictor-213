 ...
 * In terms of batch size, the higher the batch size is, the quicker
 * the sudoku boards get solved. This is especially true 
 * for the "huge" and "large" sets, but not so much for the other
 * three. "Tiny" hovers around the same speed no matter the batch size.
 * "Medium" and "small" will become faster and then hover around
 * the same speeds once the batch size reaches 256.
 * 
 * A higher batch size will allow us to test more sudokus at the same time,
 * leading to this faster run time. The speed begins to hover around 
 * certain time when the batch size is at least or greater than the
 * size of the actual set (we essentially solve all the sudokus in
 * the set at once, saving time).
 * 
 * We have chosen 10000 as our batch size because it optimizes
 * the speed for the "huge" and "large" sets. It doesn't necessarily
 * make the other three sets any faster, but they will still run
 * at around the same speed as smaller batches, so it
 * doesn't make it any slower either. Going above 10000
 * leads to potential errors, so we will avoid that.
 */
// The number of boards to pass to the solver at one time
#define BATCH_SIZE 10000

// Citation helped us understand how to declare a dim3 variable
// Citations: https://ffmpeg.org/doxygen/4.2/structuint3.html
dim3 dimBoard(9, 9);
...
/**
 * This is the kernel function that will run for each cell in 
 * our board
 *
...
__global__ void kernel(board_t* boards) {
  ...
  int row = threadIdx.x;
  int col = threadIdx.y;
  int pos = BOARD_DIM * row + col;  // check here later
  ...
  while (1) {
    ...
    if (cell_to_digit(board->cells[pos]) == 0) {
      // Loop through row
      for (int i = row * BOARD_DIM; i < (row + 1) * BOARD_DIM; i++) {
        if (cell_to_digit(board->cells[i])) {
          constraints = constraints | board->cells[i];
        }
      }

      // Loop through column
      for (int j = col; j < BOARD_DIM * BOARD_DIM; j += BOARD_DIM) {
        if (cell_to_digit(board->cells[j])) {
          constraints = constraints | board->cells[j];
        }
      }
      ...
      for (int r = box_row; r < box_row + 3; r++) {
        for (int c = box_col; c < box_col + 3; c++) {
          ...
          if (cell_to_digit(board->cells[idx]) != 0) {
            constraints = constraints | board->cells[idx];
          }
        }
      }

      // Check for change (use 0x3FE to ignore digits 10-15)
      if ((~constraints & 0x3FE) != board->cells[pos]) {
        board->cells[pos] = board->cells[pos] & (~constraints & 0x3FE);
        changed = 1;
      }
    }

    // if there is no change leave
    if (!__syncthreads_count(changed)) break;
  }
}
...
void solve_boards(board_t* boards, size_t num_boards) {
  // space for boards on gpu
  board_t* gpu_board = NULL;

  // Allocate space for the x array on the GPU
  if (cudaMalloc(&gpu_board, sizeof(board_t) * num_boards) != cudaSuccess) {
    fprintf(stderr, "Failed to allocate board pointer array on GPU\n");
    exit(2);
  }

  // Copy the cpu's board array to the gpu with cudaMemcpy
  if (cudaMemcpy(gpu_board, boards, sizeof(board_t) * num_boards, cudaMemcpyHostToDevice) !=
      cudaSuccess) {
    fprintf(stderr, "Failed to copy boards to the GPU\n");
  }

  // Run our kernel
  kernel<<<num_boards, dimBoard>>>(gpu_board);

  // Synchronize threads
  cudaDeviceSynchronize();

  // Copy the boards array back from the gpu to the cpu
  if (cudaMemcpy(boards, gpu_board, sizeof(board_t) * num_boards, cudaMemcpyDeviceToHost) !=
      cudaSuccess) {
